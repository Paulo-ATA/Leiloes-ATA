"""
RAIO-X DE LEILÕES (DIAGNÓSTICO)
-------------------------------------------------------------------
Este script não salva no banco. Ele apenas baixa o HTML bruto da
Mega Leilões e procura por valores escondidos, imprimindo no console.
"""
...
def executar_raiox():
    ...
    try:
        ...
        # Procura qualquer "R$" e captura 80 caracteres antes e 80 caracteres depois
        matches = re.finditer(r".{0,80}R\$.{0,80}", html)
        encontrou = False

        for i, match in enumerate(matches):
            if i >= 20: # Limita a 20 resultados para não poluir o seu log
                print("... (Muitos resultados encontrados, exibindo apenas os 20 primeiros) ...")
                break
                
            texto_encontrado = match.group(0).strip()
            # Filtra sujeira de HTML básica para facilitar sua leitura no log
