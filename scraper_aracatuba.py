...
class AracatubaAuctionScraper:
    ...
    def extract_occupancy_status(text: str) -> str:
        """Inferência de ocupação via regex."""
    ...
    def fetch_aracatuba_lots(self) -> List[Dict]:
        """
        Espaço reservado para futuros extratores do TRT-15.
        """
        logging.info("Iniciando varredura por leilões de Araçatuba/SP no TRT-15...")
        # Retorna lista vazia até implementarmos o extrator real do TRT-15
        return []

    def normalizar_lotes_tjsp(self, imoveis_raw_tjsp: List[Dict]) -> List[Dict]:
        """Normaliza os dados coletados do módulo TJSP / Mega Leilões."""
        lotes_normalizados = []
    ...
    def fetch_all_lots(self) -> List[Dict]:
        """Executa a busca unificada de leilões reais."""
...
def salvar_no_banco(lotes: list):
    """Insere ou atualiza (upsert) os leilões reais coletados no Supabase."""
